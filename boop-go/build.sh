#!/bin/bash
#
# Boop - Build Script for Go (macOS, Linux, and Windows)
# 用途：构建和打包桌面应用
#
# Usage:
...
go_build() {
    ...
    verbose_log "执行: go build -ldflags '${ldflags}' -o ${DIST_DIR}/${output_name} ."

    # 执行构建，编译根目录的 main.go 文件
    cd "${SCRIPT_DIR}" && go build -ldflags "${ldflags}" -o "${DIST_DIR}/${output_name}" . || {
...
create_macos_app_bundle() {
    ...
    # 复制 scripts 和 data 目录到 Resources
    if [[ -d "${SCRIPT_DIR}/scripts" ]]; then
        cp -r "${SCRIPT_DIR}/scripts" "${resources_dir}/scripts"
    fi

    if [[ -d "${SCRIPT_DIR}/data" ]]; then
        cp -r "${SCRIPT_DIR}/data" "${resources_dir}/"
    fi

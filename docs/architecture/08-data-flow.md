...
- **事务边界**：无。脚本失败时返回原文本（[utils.py#L80-L84](file:///Users/huji/work/MyProject/code_mine/gitee/boop/app/core/utils.py#L80-L84)），编辑器不修改。
...
### 标注
- **性能优化**：[script.py#L69-L71](file:///Users/huji/work/MyProject/code_mine/gitee/boop/app/core/script.py#L69-L71) 批量写盘一次，避免每个文件一次 I/O。
- **失效策略**：`mtime >= file_mtime` 视为有效（[cache.py#L78](file:///Users/huji/work/MyProject/code_mine/gitee/boop/app/core/cache.py#L78)），同秒内编辑可能误判。
...
### 标注
- **线程安全**：[global_hotkey.py#L89-L92](file:///Users/huji/work/MyProject/code_mine/gitee/boop/app/core/global_hotkey.py#L89-L92) 正确使用 `root.after(0, callback)` 跨线程调度，避免直接在监听线程操作 Tk。
- **开关**：`config.enable_global_hotkeys` 控制是否启用（[settings.py#L76](file:///Users/huji/work/MyProject/code_mine/gitee/boop/app/config/settings.py#L76)）。
...
### 标注
- **依赖来源**：脚本在 docstring JSON 中声明 `dependencies: ["json5", "yaml"]`（见 [format_json.py#L8](file:///Users/huji/work/MyProject/code_mine/gitee/boop/scripts/format_json.py#L8)）。
...
- **可观测性缺口**：Journey 1 的子进程 stderr 仅写入主进程日志，未在 UI 呈现给用户；脚本作者的 `post_info`/`post_error` 输出走 stderr 但被主进程当作字符串接收（[script_wrapper.py#L28-L31](file:///Users/huji/work/MyProject/code_mine/gitee/boop/app/core/script_wrapper.py#L28-L31)），未结构化解析。
